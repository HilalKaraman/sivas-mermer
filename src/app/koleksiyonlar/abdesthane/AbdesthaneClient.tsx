...
export default function AbdesthaneClient() {
    const [currentPage, setCurrentPage] = useState(1);
    const sectionRef = useRef<HTMLElement>(null);
    const itemsPerPage = 6;

    const imageNumbers = [1, 2, 3, 5, 6];
    const products = imageNumbers.map((num, i) => ({
        code: `AB0${(i + 1).toString().padStart(2, '0')}`,
        title: "Abdesthane Tasarımı",
        image: img(`/abdesthane${num}.jpg`),
        desc: "Doğal mermerden üretilmiş, geleneksel mimariye uyumlu estetik ve fonksiyonel abdesthane uygulaması.",
        ...
        <section ref={sectionRef} className="py-24 bg-[var(--cream-dark)] relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold text-[var(--charcoal)] mb-4">Abdesthane Uygulamaları</h2>
                    <div className="w-24 h-1 bg-[var(--gold)] mx-auto rounded-full"></div>
                    <p className="mt-4 text-[var(--grey-medium)] max-w-2xl mx-auto">
                        Geleneksel mimariye sadık kalarak doğal mermerden tasarlanan, estetik ve fonksiyonel abdesthane projeleri.
                    </p>
                </div>
