...
export default function MermerDosemeClient() {
    const [currentPage, setCurrentPage] = useState(1);
    const sectionRef = useRef<HTMLElement>(null);
    const itemsPerPage = 6;

    const products = Array.from({ length: 27 }).map((_, i) => ({
        code: `D0${(i + 1).toString().padStart(2, '0')}`,
        title: "Mermer Zemin Döşeme",
        image: getImageUrl(`doseme-urun${i + 1}.jpg`),
        desc: "Mekanlarınıza asalet ve ferahlık katan, uzun ömürlü ve dayanıklı mermer zemin döşeme uygulaması.",
